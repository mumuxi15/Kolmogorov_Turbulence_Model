...
#                count=0
                ...
                py.figure(2)
                py.imshow(log(F2.real*F2.real+F2.imag*F2.imag))
                py.draw()
                ...
                global sum_F3;
                F3=np.diag(F2)[257::]
                sum_F3=sum_F3+F3.real*F3.real+F3.imag*F3.imag
#                print size(F_3)
                axis3=log(np.arange(1,256,1))
                plt.plot(axis3,axis3*(-11.0/3)+18.0,c='g',label='prediction')
                plt.plot(axis3,log(sum_F3/(count/100.0)),c='r',label='experiments')
                plt.xlabel('frequency')
                plt.ylabel('power')
                plt.legend(bbox_to_anchor=(0.72, 1), loc=2, borderaxespad=0.)
                py.draw()
                ...
                phi=sum(l_var)/len(l_var)
                print "phi is ", phi 
                py.figure(9)
                py.clf()
                
                axis4=np.arange(0,256,1)
                py.plot(axis4*math.sqrt(2),6.88*(axis4*math.sqrt(2)/512.0)**(5.0/3),c='dodgerblue',label='theory')
                py.plot(axis4,F5[256,256:512]/count,c='green',label='horizontal')
                py.plot(axis4*math.sqrt(2),np.diag(F5)[256:512]/count,c='red',label='diagonal')
                py.plot(axis4,F5[256:512,256]/count,c='salmon',label='vertical')
